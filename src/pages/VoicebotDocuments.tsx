...
import { Upload, Search, FileText, Eye, Trash2, Loader2, RefreshCw, Mic } from "lucide-react";
...
const VoicebotDocuments = () => {
  ...
  useEffect(() => {
    const fetchBots = async () => {
      ...
      try {
        ...
        // Filter for voice bots only (channelType === 'VOICE')
        const voiceBots = botsList.filter(bot => bot.channelType === 'VOICE');
        setBots(voiceBots);

        // Auto-select first bot if available
        if (voiceBots.length > 0) {
          setSelectedBotId(voiceBots[0].botId);
        }
      } catch (err) {
        console.error('Error fetching bots:', err);
        toast({
          title: "Error",
          description: "Failed to load voice bots. Please try again.",
          variant: "destructive",
        });
      } finally {
        setLoadingBots(false);
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (!selectedBotId) {
      toast({
        title: "Select a Voice Bot",
        description: "Please select a voice bot first before uploading documents.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleUploadFiles = async (files: File[]) => {
    ...
    if (!selectedBotId) {
      toast({
        title: "Error",
        description: "Please select a voice bot first.",
        variant: "destructive",
      });
      return;
    }
        ...
        <main className="flex-1 overflow-auto">
          ...
          <div className="p-6">
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                <h1 className="text-2xl font-bold text-foreground">Voicebot Documents</h1>
                <p className="text-muted-foreground">Manage documents for your voice bots</p>
              </div>
              <div className="flex items-center gap-3">
                ...
                <Select value={selectedBotId} onValueChange={setSelectedBotId} disabled={loadingBots}>
                  <SelectTrigger className="w-[220px]">
                    ...
                    <SelectValue placeholder={loadingBots ? "Loading bots..." : "Select a voice bot"} />
                  </SelectTrigger>
                  <SelectContent>
                    {bots.map((bot) => (
            ...
            {!selectedBotId ? (
              ...
              <div className="border-2 border-dashed border-border rounded-2xl p-12 text-center">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-foreground text-lg">Select a Voice Bot</h3>
                    <p className="text-muted-foreground">
                      Choose a voice bot from the dropdown above to view and manage its documents
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-foreground">
                        {uploading ? 'Uploading...' : `Upload Documents for ${selectedBot?.agentName || 'Voice Bot'}`}
                      </h3>
                      <p className="text-muted-foreground">
                        <button
                          type="button"
                ...
                <div>
                  ...
                  {loadingDocs ? (
                    ...
                    <div className="text-center py-12 text-muted-foreground border border-dashed border-border rounded-xl">
                      ...
                      <p className="text-sm">Upload documents to train your voice bot</p>
                    </div>
                  ) : (
...
export default VoicebotDocuments;
